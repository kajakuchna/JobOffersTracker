...
class IQMJobScraper(JobScraper):
    def parse_jobs(self, soup):
        job_cards = soup.find_all(
            "li",
            class_="border-b border-block-base-text border-opacity-15 last:border-b-0",
        )
        for card in job_cards:
            job = {}
            job["title"] = (
                card.find("span", class_="text-block-base-link company-link-style")
                .get_text()
                .strip()
            )
            department_span = card.find("span", class_="text-base").find("span")
            job["department"] = (
                department_span.get_text().strip()
                if department_span
                else "No department"
            )
            ...
            job["location"] = (
                location_span.get_text().strip() if location_span else "No location"
            )

            link_element = card.find(
                "a",
                class_="hover:bg-block-base-text hover:bg-opacity-3 flex flex-col py-2 px-4",
            )
...
class XanaduJobScraper(JobScraper):
    def parse_jobs(self, soup):
        ...
        for item in job_items:
            ...
            job["location"] = (
                location_element.get_text().strip() if location_element else "No data"
            )
            department_element = item.find_previous("div", class_="department-heading")
            job["department"] = (
                department_element.find("h4").get_text().strip()
